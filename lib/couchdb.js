...
CouchDB.prototype.createAttachment = async function(model, data, options) {
  debug('CouchDB.prototype.uploadFile %j %j', model, data);
  ...
  assert(data.file, "file property missing in model");
  const file = data.file;
  assert(file.mimetype, "mimetype missing on file object");
  assert(file.buffer, "buffer type missing on file object");
  delete data.file;

  debug('`Filename: ${filename} with id=${doc.id}`', model, data);

  assert(data.filename, "filename property missing in model");
  const filename = data.filename;
  delete data.filename;

  const doc = await db.insert(data, options);
  await db.attachment.insert(doc.id, filename, file.buffer, file.mimetype, {rev: doc.rev});

...
function CouchDB (name, settings, ds) {
  // Injection for tests
  this.CouchDBDriver = settings.Driver;

  assert( this.CouchDBDriver, 'configured nano driver required here');

  debug('***** COUCH-ACL CouchDB constructor settings: %j', settings);
...
CouchDB.prototype.connect = function ( cb ) {
  debug('***** COUCH-ACL Invoking patched connect method via CouchDB connector');
  const self = this;
  self.couchdb = self.CouchDBDriver;
...
CouchDB.prototype.attachmentAsStream = async function(options) {
  ...
  const stream = await db.attachment.getAsStream(options.id, options.filename);

...
CouchDB.prototype.connect = async function(cb) {
  ...
  const dbName = self.options.database;
  assert(dbName, "DataBase name missing");

  try {
    await self.couchdb.db.get(dbName);
    debug('Database exists', dbName);
    self.dataSource.emit('couchdb.connector.db_exists');
    return cb(null, self.couchdb);
  } catch (err) {
    if (err.error === 'not_found') {
      if (this.options.databaseCreate === true) {
        debug('Creating database', dbName);
        try {
          await self.couchdb.db.create(dbName);
        ...
        self.dataSource.emit('couchdb.connector.error');
        return cb(null, self.couchdb);
      }
    }
    return cb(err);
...
const assertFile = file  => {
  assert(file.hasOwnProperty("mimetype"), "property mimetime missing");
  assert(file.hasOwnProperty("buffer"), "property buffer missing");
  assert(file.mimetype, "mimetime empty");
  assert(file.buffer, "buffer empty");
};

CouchDB.prototype.createAttachment = async function(options, file) {
  debug('CouchDB.prototype.createAttachment %j', options);
  assertFile(file);

  const self = this;
  const db = await this.couchdb.use(self.getDbName(self));

  debug(`Filename: ${options.filename} with id=${options.id}`);
  await db.attachment.insert(options.id, options.filename, file.buffer, file.mimetype,
      options.rev ? {rev: options.rev} : {});
};


CouchDB.prototype.attachmentAsStream = async function(options) {
  debug('CouchDB.prototype.createAttachment %j', options);
  const self = this;
  const db = await this.couchdb.use(self.getDbName(self));
  const doc = await db.get(options.id);
  const stream = db.attachment.getAsStream(options.id, options.filename);
  const {content_type} = doc._attachments[options.filename];
  return {contentType: content_type, stream};
}
